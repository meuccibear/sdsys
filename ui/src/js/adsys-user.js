	...
	$('#appusers-table').bootstrapTable({
		...
		ajax: ajaxAppusersRequest,
                columns: [{
                    ...
                    align: 'center',
		    formatter:function(value,row,index){ 
			return userTypeFormat(row.usertype);
		    }
                },{
                    title: '最近登录时间',
                    field: 'logindate',
			...
			if (row.logindate)
	            ...
	            formatter:function(value,row,index){  
			var html = '';
	                html = html + '<button title="修改" class="btn btn-warning btn-xs" onclick="showEditAppuserModal(\'' + row.uid + '\');"><i class="fa fa-edit"></i></button>';
                    	html = html + '  <button title="删除" class="btn btn-danger btn-xs" onclick="delAppuser(\'' + row.uid + '\');"><i class="fa fa-trash-o"></i></button>';
